package com.example.cook.global.login.filter;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
  ...
  public CustomJsonUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
    /**
     * super(): 부모 클래스의 생성자 호출, 부모 클래스인 AbstractAuthenticationProcessingFilter()의 생성자 파라미터로 위에서 상수로 선언한 "/login" URL을 설정
     * 필터는 "/login" URL이 들어올 시 작동
     */
  ...
  public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
    // 요청의 ContentType이 null이거나 우리가 설정한 application/json이 아니라면,AuthenticationServiceException 예외를 발생시켜 JSON으로 요청
    ...
    // JSON 요청을 String으로 변환한 messageBody를 objectMapper.readValue()를 통해 Map으로 변환하여 각각 Email, Password를 저장
    ...
    /**
     * FormLogin 필터인 UsernamePasswordAuthenticationFilter와 동일하게 UsernamePasswordAuthenticationToken 객체를 사용
     * 이 객체는 인증 처리 객체인 AuthenticationManager가 인증 시 사용할 인증 대상 객체가 됩니다.
     * 파라미터로 넘겨준 email이 인증 대상 객체의 principal, password가 인증 대상 객체의 credentials이 됩니다.
     * 여기서 만든 UsernamePasswordAuthenticationToken 인증 대상 객체를 통해, 인증 처리 객체인 AuthenticationManager가 인증 성공/인증 실패 처리를 하게 됩니다.
     */
